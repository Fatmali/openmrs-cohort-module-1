...
public class AddCohortController {
    ...
    private void addReferenceData(ModelMap model) {
        ...
        List<CohortProgram> list2 = service1.getAllCohortPrograms();
    ...
    public String onSubmit(WebRequest request, HttpSession httpSession, HttpServletRequest request1,
        ...
        if (errors.hasErrors()) {
            ...
            CohortProgram list2 = departmentService.getCohortProgramByName(cohort_program);
            CohortType cohorttype1 = departmentService.getCohortTypeByName(cohort_type_name);
            LocationService service = Context.getLocationService();
    ...
    public void manage1(ModelMap model, HttpSession httpSession, HttpServletRequest request, @RequestParam("cpid") Integer id, @ModelAttribute("cpatient") CohortMember cohort) {
        ...
        CohortService departmentService = Context.getService(CohortService.class);
        CohortM cohort1 = departmentService.getCohortById(id);
        if (cohort1 != null) {
            CohortM cohort2 = cohort1;
            String cname = cohort2.getName();
            CohortRole cr = departmentService.getCohortRoleByName(cname);
            if (cr != null) {
                CohortRole c3 = cr;
                type.add(c3.getName());
            }
    ...
    public void onClick(WebRequest request, HttpSession httpSession, ModelMap model,
        ...
        model.addAttribute("cohort", departmentService.getCohortById(id));
        patient = ps.getPatient(pid);
        String rolname = request.getParameter("format");
        CohortRole crole = departmentService.getCohortRoleByName(rolname);
        if (crole != null) {
            c2 = crole;
        }
        boolean repeatPatient = false;
        for (CohortMember cohortMember : departmentService.getCohortMembersByCohortId(id)) {
        ...
        Date parsedStartDate = new SimpleDateFormat("dd/MM/yyyy", Locale.ENGLISH).parse(startDate);
        Date parsedEndDate = new SimpleDateFormat("dd/MM/yyyy", Locale.ENGLISH).parse(endDate);
    ...
    public void manage2(ModelMap model, HttpSession httpSession, HttpServletRequest request, @ModelAttribute("cpatient") CohortMember cohortmem, @ModelAttribute("cohortmodule") CohortM cohort) {
        ...
        for (int i = 0; i < cp.size(); i++) {
            ...
            CohortRole cr = service1.getCohortRoleByName(c.getName());
            if (cr != null) {
                CohortRole c3 = cr;
                type.add(c3.getName());
            }
    ...
    public String onSubmit1(WebRequest request, HttpSession httpSession, HttpServletRequest request1,
        ...
        CohortM cc = service1.getCohortByName(cohort_name1);
        if (cc != null) {
            CohortM c1 = cc;
        ...
        patient = ps.getPatient(pid);
        String rolname = request.getParameter("format");
        CohortRole crole = service1.getCohortRoleByName(rolname);
        if (crole != null) {
            c2 = crole;
        }
